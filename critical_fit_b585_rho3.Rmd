## critical fit W $\beta=5.85$ $\rho=3$ {#critical_fit_W_beta585_rho3}
...
### tau=2 {#critical_fit_W_beta585_rho3_tau2}
...
```{r , child="../fit_all_out/r_awi_loc_tau2_critical_b585_rho3_simple_fit_P.tex", eval=TRUE}
```



```{r, echo=FALSE, results='asis'}
file=sprintf("../fit_all_out/r_awi_loc_tau2_critical_b585_rho3_simple_fit_data.txt")
...
for (en in c(1:length(df0[,1])) ){
  ...
  file=sprintf("../fit_all_out/r_awi_loc_tau2_critical_b585_rho3_simple_fit_out_n%d_en%d_eta.txt",n,en-1)
...
## 7par fit local rawi {#fit_7par_rawi_W_585_rho3}
...
\begin{cases}
r_{AWI}^{loc}=P[2]  (\eta- \eta_{cr})  +P[3] (m_0-m_{cr}) \\
m_{PCAC}^{loc}= P[4]  (\eta- \eta_{cr}) +P[5] (m_0-m_{cr}) + P[6] \mu\,.
\end{cases}
$$
In the above fit we are treating $\eta_{cr}$ and $m_{cr}$  as fits parameters, so
$$
...
```{r , child="../fit_all_out/r_awi_loc_tau2_critical_b585_rho3_7par_fit_P.tex", eval=TRUE}
```



```{r, echo=FALSE, results='asis'}
file=sprintf("../fit_all_out/r_awi_loc_tau2_critical_b585_rho3_7par_fit_data.txt")
dfn<- read.table(file, header=FALSE, fill=TRUE)
#names(df) <- c("$L$","$k/m$","err", "$T$","$q2$")
#df<- df[c("$T$","$L$","$k/m$","err" ,"$q2$")] 
#kable(df[,-5],digit=20)
...
for (en in c(1:length(df0[,1])) ){
  ...
  file=sprintf("../fit_all_out/r_awi_loc_tau2_critical_b585_rho3_7par_fit_out_n%d_en%d_eta.txt",n,en-1)
  datalist[[en]]<- read.table(file, header=FALSE, fill=TRUE,   
                  col.names=c(paste0("x",n),paste0("fit",n),paste0("fiterr",n)))
