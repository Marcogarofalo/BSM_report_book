## critical fit NG $\beta=5.85$ $\rho=3$ {#critical_fit_NG_beta585_rho3}
...
$$
\begin{cases}
m_{PCAC}=P[0] + P[2] (\eta- \eta_{cr})  + P[4] \mu+ P[6](m_0- m_{cr})\\
M_{PS}^2=P[1] + P[3] (\eta- \eta_{cr})  + P[5] \mu+ P[7](m_0- m_{cr})+ P[8]  (\eta- \eta_{cr})^2\,.
\end{cases}
$$
...
```{r , child="../fit_all_out/fit_NG_mpcac_MPS_b585_rho3_lines_fit_P.tex", eval=TRUE}
```
...
n1<-which(dfn[,1]==1)
df1<-dfn[n1,]
gg<- ggplot()+scale_shape_manual(values=c(0:50))
...
for (en in c(1:length(df0[,1])) ){
  ...
  file=sprintf("../fit_all_out/fit_NG_mpcac_MPS_b585_rho3_lines_fit_out_n%d_en%d_eta.txt",n,en-1)
...
fig_pdf<- myplotly(gg,"", "$\\eta$", "$m_{pcac}$",  output="AUTO",to_print = FALSE)%>% add_annotations( text="$(\\mu_3,\\color{red}{m}_{\\color{cyan}0})$", xref="paper", yref="paper",
                  x=1.10, xanchor="left",
...
color=as.factor(df1[en,10]), fill=as.factor(df1[en,10]),
                          shape=as.factor("ribbon")                                                                ),alpha=0.5,  )
 
}

gg<- gg+theme_bw() #+ ggplot2::coord_cartesian(xlim=c(0,0.05), ylim=c(-3,0 ))
gg<- gg+labs(color = "", shape= "")
fig_pdf<- myplotly(gg,"", "$\\eta$", "$M_{PS}^2$",  output="AUTO",to_print = FALSE)%>% add_annotations( text="$(\\mu_3,\\color{red}{m}_{\\color{cyan}0})$", xref="paper", yref="paper",
                  x=1.10, xanchor="left",
                  y=0.75, yanchor="bottom",    # Same y as legend below
                  legendtitle=TRUE, showarrow=FALSE ) %>%
  layout( legend=list(y=0.7, yanchor="top",title=F ) )
